...
* `Building and Running an edX Course`_ 
  ...
  * Updated `Grade and Answer Data`_ section to include new features on
    the problem Staff Debug viewer for rescoring, resetting attempts, and
    and deleting state for a specified student.
  ...
  * New `VitalSource E-Reader Tool`_ section.
...
* `edX Data Documentation`_
  ...
  Updated `Tracking Logs`_ section that includes events for course
...
* For researchers:
  ...
  There are two new events for course enrollment activities:
  ``edx.course.enrollment.activated`` and ``edx.course.enrollment.deactivated``.
  See `Tracking Logs`_ for more information
